...
- [[#perspective-mode][Perspective Mode]]
  - [[#cleanup-on-frame-delete][Cleanup on frame delete]]
...
Install compat, emacs may be outdated.
...
Take a look at the [[https://github.com/bbatsov/asciidoc-mode][git repo]] for details.
...
More information can be found on [[https://kdl.dev/][kdl.dev]].
...
** Cleanup on frame delete

Opening a new frame and perspective may result in a bunch of open buffers inside that frame.
These buffers should be killed, when the frame and perspective is killed.
All buffers associated uniquely in that frame must be gathered and killed before the frame.
The following functions do exactly that.
